...
class Swiping extends React.Component {
  ...
  onDoubleTap(restaurant) {
    ...
    if (!restaurantsArray[restaurantIndex].additionalPhotos) {
      ...
      restaurantsArray[restaurantIndex].photoURL =
        additionalPhotos[curPhotoIndex];
      this.setState({ restaurants: restaurantsArray });
    }
    ...
    socket.on("match found", (placeID) => {
      for (let i = 0; i < this.state.restaurants.length; i++) {
        if (this.state.restaurants[i].placeID === placeID) {
          this.setState({
            matchedRestaurant: this.state.restaurants[i],
            matchOpen: true,
          });
          break;
        }
  ...
  render() {
    ...
    for (
      let word = 0;
      word < googleDirectionParameterArray.length - 1;
      word++
    ) {
      googleDirectionsParameter += googleDirectionParameterArray[word] + "+";
    }
    googleDirectionsParameter +=
      googleDirectionParameterArray[googleDirectionParameterArray.length - 1];
          ...
          {this.state.restaurants.map((restaurant) => (
            ...
            >
              ...
              <div className="restaurantDistance">
                <h5>
                  {Unit(
                    convert(restaurant.distance)
                      .from("m")
                      .to(getUnitSetting() === "metric" ? "m" : "mi")
                      .toFixed(2),
                    { metric: "m", imperial: "mi" }
                  )}
                </h5>
              </div>
            </Card>
            ...
            <div>
              ...
              >
                <img
                  ...
                  style={this.restaurantPhotoStyle}
                  alt="Restaurant"
                />
              </a>
              <div className="restaurantRatingPrice">
